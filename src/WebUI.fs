...
let private getLibName () =
    if RuntimeInformation.IsOSPlatform(OSPlatform.Windows) then "webui-2.dll"
    elif RuntimeInformation.IsOSPlatform(OSPlatform.OSX) then "libwebui-2.dylib"
    else "libwebui-2.so"

let private initLibResolver =
    let expectedLib = getLibName()
    let resolver (name: string) _ _ =
        if name = "libwebui-2.so" || name = "webui-2.dll" || name = "libwebui-2.dylib" then
            let thisAssembly = Assembly.GetExecutingAssembly()
            let dir = System.IO.Path.GetDirectoryName(thisAssembly.Location)
            let fullPath = System.IO.Path.Combine(dir, expectedLib)
            if System.IO.File.Exists(fullPath) then NativeLibrary.Load(fullPath)
            else IntPtr.Zero
        else IntPtr.Zero
...
let newWindow () = NativeMethods.webui_new_window()
let newWindowId id = NativeMethods.webui_new_window_id id
let getNewWindowId () = NativeMethods.webui_get_new_window_id()
let destroy w = NativeMethods.webui_destroy w

let show (w: uint) (html: string) =
    NativeMethods.webui_show(w, html) |> ignore; true

let showBrowser w c b = NativeMethods.webui_show_browser(w, c, browserToUInt b) |> ignore; true
let wait () = NativeMethods.webui_wait()
let waitAsync () = NativeMethods.webui_wait_async()
let exit () = NativeMethods.webui_exit()
let close w = NativeMethods.webui_close w
let clean () = NativeMethods.webui_clean()

let setHide w h = NativeMethods.webui_set_hide(w, h) |> ignore
let setKiosk w k = NativeMethods.webui_set_kiosk(w, k) |> ignore
let setSize w wdt hgt = NativeMethods.webui_set_size(w, wdt, hgt) |> ignore
let setPosition w x y = NativeMethods.webui_set_position(w, x, y) |> ignore
let setCenter w = NativeMethods.webui_set_center w
let setResizable w r = NativeMethods.webui_set_resizable(w, r) |> ignore
let setMinimumSize w wdt hgt = NativeMethods.webui_set_minimum_size(w, wdt, hgt) |> ignore
let setFrameless w f = NativeMethods.webui_set_frameless(w, f) |> ignore
let setTransparent w t = NativeMethods.webui_set_transparent(w, t) |> ignore
let run w s = NativeMethods.webui_run(w, s) |> ignore

let getUrl w = 
    let ptr = NativeMethods.webui_get_url w
    if ptr <> IntPtr.Zero then Marshal.PtrToStringAnsi(ptr) else ""
let navigate w u = NativeMethods.webui_navigate(w, u) |> ignore
let isShown w = NativeMethods.webui_is_shown w

let browserExist b = NativeMethods.webui_browser_exist (uint b)
let getBestBrowser w = NativeMethods.webui_get_best_browser w

let setRootFolder w p = NativeMethods.webui_set_root_folder(w, p)
let setDefaultRootFolder p = NativeMethods.webui_set_default_root_folder p
let openUrl u = NativeMethods.webui_open_url u |> ignore
let focus w = NativeMethods.webui_focus w |> ignore
let minimize w = NativeMethods.webui_minimize w |> ignore
let maximize w = NativeMethods.webui_maximize w |> ignore
let deleteProfile w = NativeMethods.webui_delete_profile w |> ignore
let getParentProcessId w = NativeMethods.webui_get_parent_process_id w
let getChildProcessId w = NativeMethods.webui_get_child_process_id w

let setTimeout s = NativeMethods.webui_set_timeout s |> ignore
let setEventBlocking w b = NativeMethods.webui_set_event_blocking(w, b) |> ignore
let setPublic w p = NativeMethods.webui_set_public(w, p) |> ignore
let getPort w = NativeMethods.webui_get_port w
let setPort w p = NativeMethods.webui_set_port(w, p)
let getFreePort () = NativeMethods.webui_get_free_port()

let setProfile w n p = NativeMethods.webui_set_profile(w, n, p) |> ignore
let setProxy w p = NativeMethods.webui_set_proxy(w, p) |> ignore
let setCustomParameters w p = NativeMethods.webui_set_custom_parameters(w, p) |> ignore
let setHighContrast w h = NativeMethods.webui_set_high_contrast(w, h) |> ignore
let isHighContrast () = NativeMethods.webui_is_high_contrast()
...
let setIcon w i t = NativeMethods.webui_set_icon(w, i, t) |> ignore
...
let runClient (e: IntPtr) (script: string) = NativeMethods.webui_run_client(e, script) |> ignore
...
let getCount (e: IntPtr) = NativeMethods.webui_get_count(e)
let getIntAt (e: IntPtr) (index: uint) = NativeMethods.webui_get_int_at(e, index)
let getInt (e: IntPtr) = NativeMethods.webui_get_int(e)
let getFloatAt (e: IntPtr) (index: uint) = NativeMethods.webui_get_float_at(e, index)
let getFloat (e: IntPtr) = NativeMethods.webui_get_float(e)
...
let getString (e: IntPtr) = 
    ...
    if ptr <> IntPtr.Zero then Marshal.PtrToStringAnsi(ptr) else ""
let getBoolAt (e: IntPtr) (index: uint) = NativeMethods.webui_get_bool_at(e, index)
let getBool (e: IntPtr) = NativeMethods.webui_get_bool(e)
let getSizeAt (e: IntPtr) (index: uint) = NativeMethods.webui_get_size_at(e, index)
let getSize (e: IntPtr) = NativeMethods.webui_get_size(e)

let returnInt (e: IntPtr) (n: int64) = NativeMethods.webui_return_int(e, n) |> ignore
let returnFloat (e: IntPtr) (f: double) = NativeMethods.webui_return_float(e, f) |> ignore
let returnString (e: IntPtr) (s: string) = NativeMethods.webui_return_string(e, s) |> ignore
let returnBool (e: IntPtr) (b: bool) = NativeMethods.webui_return_bool(e, b) |> ignore
...
let setContext (w: uint) (element: string) (context: IntPtr) = NativeMethods.webui_set_context(w, element, context) |> ignore
let getContext (e: IntPtr) = NativeMethods.webui_get_context(e)

let getLastErrorNumber () = NativeMethods.webui_get_last_error_number()
let getLastErrorMessage () = 
    let ptr = NativeMethods.webui_get_last_error_message()
    if ptr <> IntPtr.Zero then Marshal.PtrToStringAnsi(ptr) else ""

let malloc (size: uint) = NativeMethods.webui_malloc(size)
let memcpy (dest: IntPtr) (src: IntPtr) (count: uint) = NativeMethods.webui_memcpy(dest, src, count) |> ignore

let isAppRunning () = NativeMethods.webui_interface_is_app_running()
...
let interfaceGetStringAt (w: uint) (eventNumber: uint) (index: uint) =
    ...
    if ptr <> IntPtr.Zero then Marshal.PtrToStringAnsi(ptr) else ""
let interfaceGetIntAt (w: uint) (eventNumber: uint) (index: uint) = NativeMethods.webui_interface_get_int_at(w, eventNumber, index)
let interfaceGetFloatAt (w: uint) (eventNumber: uint) (index: uint) = NativeMethods.webui_interface_get_float_at(w, eventNumber, index)
let interfaceGetBoolAt (w: uint) (eventNumber: uint) (index: uint) = NativeMethods.webui_interface_get_bool_at(w, eventNumber, index)
let interfaceGetSizeAt (w: uint) (eventNumber: uint) (index: uint) = NativeMethods.webui_interface_get_size_at(w, eventNumber, index)

let interfaceSetResponse (w: uint) (eventNumber: uint) (response: string) = NativeMethods.webui_interface_set_response(w, eventNumber, response) |> ignore
let interfaceSendRawClient (w: uint) (eventNumber: uint) (functionName: string) (data: byte[]) =
    ...
    try
        ...
        Marshal.FreeHGlobal(ptr)
let interfaceRunClient (w: uint) (eventNumber: uint) (script: string) = NativeMethods.webui_interface_run_client(w, eventNumber, script) |> ignore
...
let navigateClient (e: IntPtr) (url: string) = NativeMethods.webui_navigate_client(e, url) |> ignore
let showClient (e: IntPtr) (content: string) = NativeMethods.webui_show_client(e, content) |> ignore
let closeClient (e: IntPtr) = NativeMethods.webui_close_client(e) |> ignore

let startServer (w: uint) (content: string) =
    let ptr = NativeMethods.webui_start_server(w, content)
    if ptr <> IntPtr.Zero then Marshal.PtrToStringAnsi(ptr) else ""

let showWv (w: uint) (content: string) = NativeMethods.webui_show_wv(w, content)

let setBrowserFolder (path: string) = NativeMethods.webui_set_browser_folder(path) |> ignore
let deleteAllProfiles () = NativeMethods.webui_delete_all_profiles() |> ignore
...
let interfaceSetResponseFileHandler (w: uint) (response: IntPtr) (length: int) = NativeMethods.webui_interface_set_response_file_handler(w, response, length) |> ignore

let interfaceGetWindowId (w: uint) = NativeMethods.webui_interface_get_window_id(w)
...
let setTlsCertificate (certificatePem: string) (privateKeyPem: string) = NativeMethods.webui_set_tls_certificate(certificatePem, privateKeyPem)